...
class Program2Mapper extends Mapper<LongWritable, Text, Text, FloatWritable> {
    // the mapper sends make as key and barrels08 as value (annual petroleum
    // consumption).
    ...
    public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
        ...
        if (!Make.matches("-?\\d+(\\.\\d+)?") && barrels08.matches("-?\\d+(\\.\\d+)?")) {// make sure Make is not
                                                                                         // numerical and barrels 08 is
                                                                                         // numerical.
            if (Make != null && !Make.isEmpty()) {// make sure Model and Make are not empty or null.
                if (barrels08 != null && !barrels08.isEmpty()) {
                    ResultKey.set(Make);
...
public class Program {
    public static void main(String[] args) throws Exception {
        ...
        job.setJobName("Fuel Consumption");
